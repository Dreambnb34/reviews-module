...
for (let i = 0; i < 100; i++) {
  ...
  if (rand <= 50) {
    ...
    data.neighborhoodDescription = faker.fake(
    ...
    data.gettingAroundDescription = faker.fake(
      '{{lorem.paragraph}}{{lorem.paragraph}}',
    );
  }
  ourFaker.push(data);
...
fs.writeFile(
  path.join(__dirname + '/../json/listings/listings.js'),
  JSON.stringify(ourFaker, null, '\t'),
  err => {
    if (err) {
      throw err;
    }
